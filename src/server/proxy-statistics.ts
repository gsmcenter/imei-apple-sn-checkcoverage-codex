...
export async function proxyStatistics(db: Database) {
  const rows = (
    await db.query<Record<string, string | null>>(`WITH measured AS (
    SELECT check_id,proxy,sum(apple_ms) AS ms,count(*) AS sessions,count(*) FILTER(WHERE limited) AS limits FROM apple_sessions GROUP BY check_id,proxy
  ...
  for (const dimension of ['proxy', 'concurrency', 'stage', 'hour'] as const) {
    ...
    const rs = (
      await db.query<{ name: string; sessions: string; limited: string }>(
        `SELECT ${expr} AS name,count(*) AS sessions,count(*) FILTER(WHERE limited ${dimension === 'stage' ? 'AND limit_stage=stage' : ''}) AS limited FROM apple_sessions ${dimension === 'stage' ? 'CROSS JOIN LATERAL unnest(stages) AS stage' : ''} GROUP BY ${expr} ORDER BY name DESC ${dimension === 'hour' ? 'LIMIT 168' : ''}`,
      )
    ).rows;
    limits.push(
      ...rs.map((r) => ({
