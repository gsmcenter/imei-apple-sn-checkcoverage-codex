...
CaptchaAI ma osobną kartę abonamentu: `threadsinfo` zwraca liczbę wątków i ich zajętość, bez salda USD. Prywatny endpoint `GET /api/v1/system/captchaai-account` buforuje odczyt przez minutę; brak odpowiedzi pokazuje stan niedostępny, a nie zero. Współdzielone konto obejmuje także wątki zajęte przez inne skrypty.

Klient CaptchaAI przyjmuje identyfikatory zadań zarówno jako liczbę JSON, jak i tekst; odczytuje JSON oraz odpowiedzi `OK|...`. Obraz wysyła przez multipart POST, wynik pobiera przez GET. Po jednoznacznym odrzuceniu zgłoszenia `ERROR_ZERO_BALANCE` lub `ERROR_NO_SLOT_AVAILABLE` czeka na dostępność abonamentu maksymalnie 60 s (w ramach `CAPTCHA_TIMEOUT_MS`). Kod `ERROR_ZERO_BALANCE` sam nie rozróżnia zajętości od problemu z abonamentem — sprawdź liczniki w panelu. Nie ponawia zgłoszenia po niepewnym błędzie sieci ani po otrzymaniu ID; odpytuje ten sam wynik. Błędy klucza, IP, połączenia i odpowiedzi mają osobne komunikaty. Klucz pozostaje w `CAPTCHAAI_API_KEY`; nie potrzeba nowych zmiennych Railway.
